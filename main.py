...
def start(message):
	...
	if f == SUDO:
		...
		M = types.InlineKeyboardButton('DEV', url='https://t.me/E_M_K')
		mas.add(G,K)
		mas.add(V)
		mas.add(M)
		...
		me = types.InlineKeyboardButton(text="KHALID",url="https://t.me/E_M_K")
...
def masg(call):
	...
	if call.data =="E_M_K":
		...
		while True:
			...
			E_M_K = [h1,h2,h3,h4,h5]
			j = random.choice(E_M_K)
			url = "https://t.me/"+j
			headers = {
            "User-Agent": generate_user_agent(),
            "Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8",
			...
			if response.text.find('If you have <strong>Telegram</strong>, you can contact <a class="tgme_username_link"')>=0 and j not in ban:
				...
				R = types.InlineKeyboardButton(f'{sk}', callback_data="1x")
				E_M_K = types.InlineKeyboardButton('DEV', url='https://t.me/E_M_K')
				mas.add(A,E,B,R,E_M_K)
				bot.edit_message_text(chat_id=call.message.chat.id,message_id=call.message.message_id,text="ok start",reply_markup=mas)
